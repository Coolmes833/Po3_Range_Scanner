#!/usr/bin/env python3
"""
Po3 / AMD monitor v5 — Binance USDT-M Futures, stdlib only (Python 3.6+).

v5: adds DRAW ON LIQUIDITY detection — unswept swing highs above the range
(buy-side liquidity) for longs, unswept swing lows below (sell-side) for
shorts. Equal highs/lows (EQH/EQL) earn extra points. Optional hard filter:
--require-draw. The nearest liquidity pool is reported as the draw target.
...
CSV_FIELDS = ["alert_ts", "alert_time", "symbol", "tf", "side", "score",
              "range_low", "range_high", "sweep", "entry", "target", "draw",
...
def find_liquidity_pools(candles, side, k=2):
    """
    Unswept liquidity pools (draw on liquidity).
    LONG : swing highs never exceeded afterwards -> resting buy-side liquidity
    SHORT: swing lows never undercut afterwards  -> resting sell-side liquidity
    Swing = fractal: higher/lower than k neighbours on both sides.
    ...
    for i in range(k, n - k):
        c = candles[i]
        if side == "long":
            if (all(c["h"] > candles[j]["h"] for j in range(i - k, i)) and
                    all(c["h"] > candles[j]["h"] for j in range(i + 1, i + k + 1))):
                if all(candles[j]["h"] < c["h"] for j in range(i + k + 1, n)):
                    pools.append(c["h"])
        else:
...
def check_draw(candles, side, res, eq_tol=0.002):
    """
    Look for unswept liquidity BEYOND the range in the expansion direction.
    Returns (nearest_pool, is_equal_cluster) or (None, False).
    """
    pools = find_liquidity_pools(candles, side)
    if side == "long":
        beyond = sorted(p for p in pools if p > res["range_high"])
    else:
        beyond = sorted((p for p in pools if p < res["range_low"]),
                        reverse=True)
    if not beyond:
        return None, False
    nearest = beyond[0]
    # equal highs/lows: two pools within eq_tol of each other = stronger magnet
    eq = any(
        abs(beyond[i] - beyond[j]) / nearest <= eq_tol
        for i in range(len(beyond)) for j in range(i + 1, len(beyond))
    ) if len(beyond) > 1 else False
    return nearest, eq
    ...
    for tf in args.tfs:
        ...
        for sym in symbols:
            ...
            for side in ["long", "short"]:
                ...
                # --- draw on liquidity (unswept highs/lows beyond range) ---
                draw, eq = check_draw(candles, side, res)
                res["draw"] = draw if draw else ""
                if args.require_draw and not draw:
                    continue
                if draw:
                    res["score"] += 1.0
                    tag = "BSL" if side == "long" else "SSL"
                    res["flags"].append("{}@{}".format(tag, draw))
                    if eq:
                        res["score"] += 0.5
                        res["flags"].append("EQH" if side == "long" else "EQL")
...
def main():
    ...
    ap.add_argument("--require-draw", action="store_true",
                    help="alert ONLY if unswept liquidity exists beyond the "
                         "range in the expansion direction")
